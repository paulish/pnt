...
const getReplies = require('./replies');
...
const cultureHandler = (sender, message) => {
    if (!message.author.bot && message.channel.id === sender.textHandlerChannel) {
        const reply = sample(getReplies(message.content));
...
module.exports = {
    addCommands: (commands) => {
        ...
        commands.builtIn.push({
            name: 'гопник',
            help: 'включить режим культурного общения',
            handler: (action) => {
                commands.textHandler = cultureHandler;
                commands.textHandlerChannel = action.message.channel.id;
                return action.message.channel.send(constants.GREETING);
            }
        });

        commands.builtIn.push({
            name: 'заткнись',
            help: 'выключить режим культурного общения',
            handler: (action) => {
                commands.textHandler = null;
                return action.message.channel.send('Да пошли вы :(');
            }
        });
    }
}