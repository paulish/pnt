        ...
        commands.builtIn.push({
            ...
            handler: (action) => {
                ...
                switch (action.type) {
                    ...
                    case 'hulls': pr.push(db.getHulls()); break;
                }
                Promise.all(pr).then(([players, data]) => {
                    let engine = getImageEngine();
                    let image;
                    switch (action.type) {
                        case 'module': image = engine.getModuleCross(players, data, rankColor); break;
                        case 'turret': image = engine.getTurretCross(players, data, rankColor); break;
                        case 'hulls': image = engine.getHullCross(players, data, rankColor); break;
                    }
                    return image.then(canvas => action.message.channel.send(new Discord.Attachment(canvas.createPNGStream())));
                })
            }
        });
    }
