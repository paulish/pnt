...
class db {
    ...
    getRating(type) {
        return sqlite.all(
            `SELECT case when p.tag is not null then '[' || p.tag || '] ' else '' end || p.name name, r.idplayer, r.${type}, r.old_${type}
            FROM player p JOIN player_rating r ON (p.id = r.idplayer)
            ORDER BY r.${type} DESC, r.old_${type} DESC`);
    }
    ...
    setTag(playerId, tag) {
        return sqlite.run('update player set tag = ? where id = ?', tag === '-' ? null : tag, playerId);
    }
